...
input_vector_size = symbol_count * 2 if args.bootstrap_in else symbol_count
...
with tf.Session() as session:
  cudnn_param_size_v = cudnn_param_size.eval()
cudnn_params = tf.Variable(tf.random_normal([cudnn_param_size], mean=0.0, stddev=0.02), validate_shape=False, name="cudnn_params")
...
def build_input_vector(input_sequence, ngram_input_sequence, ngram_predictions):
  ...
  if args.bootstrap_mem:
    input_list.append(tf.gather(ngram_embeddings, ngram_input_sequence[:,:,3]))
...
class Train:
  with tf.name_scope('Train'):
    ...
    outputs = tf.batch_matmul(outputs, tiled_projection_matrix) + output_projection_b
    ...
    cudnn_params.get_shape().as_list = lambda: [cudnn_param_size_v]
    ...
    with tf.control_dependencies([apply_gradients, final_batch_progress]):
      ...
      for state, out in zip([network_state_c, network_state_h], [output_c, output_h]):
        op = tf.assign(state, tf.select(expanded_finished_batch_mask, initial_state, out))
